 ...
 * Orchestration: validation → création agrégat → persistance → events
 */
@Singleton
class CreateMaltCommandHandler @Inject()(
                                          maltReadRepo: MaltReadRepository,
                                          maltWriteRepo: MaltWriteRepository
                                          // originRepository: OriginReadRepository // À ajouter quand disponible
  ...
  private def createMalt(command: CreateMaltCommand): Future[Either[DomainError, MaltId]] = {
    ...
    val valueObjectsResult = for {
      ...
      maltType <- MaltType.fromName(command.maltType)
        .toRight(s"Type de malt invalide: ${command.maltType}")
      ebcColor <- EBCColor(command.ebcColor)
      extractionRate <- ExtractionRate(command.extractionRate)
      diastaticPower <- DiastaticPower(command.diastaticPower)
      source <- MaltSource.fromName(command.source)
        .toRight(s"Source invalide: ${command.source}")
    ...
    valueObjectsResult match {
      ...
      case Right((name, maltType, ebcColor, extractionRate, diastaticPower, source)) =>
        getOriginAndCreateMalt(name, maltType, ebcColor, extractionRate, diastaticPower, source, command)
    }
  }

  private def getOriginAndCreateMalt(
                                      name: NonEmptyString,
                                      maltType: MaltType,
                                      ebcColor: EBCColor,
    ...
    // Pour simplification, création origine basique
    val origin = Origin(
      code = command.originCode,
      name = command.originCode, // Simplifié
      region = "Unknown",
      isNoble = false,
      isNewWorld = false
    )
