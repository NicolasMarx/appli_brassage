...
class MaltPublicController @Inject()(
  ...
  private def buildSearchSuggestions(malts: List[MaltAggregate], filter: Any): List[String] = {
    malts.take(5).map(_.name.value)
  }
  
  private def buildMaltFacets(malts: List[MaltAggregate]): MaltFacetsDTO = {
    MaltFacetsDTO(
      maltTypes = List.empty,
      sources = List.empty,
      colorRanges = List.empty,
      extractionRanges = List.empty,
      flavorCategories = List.empty
    )
  }
}